      ...
      >
        ...
        {form.getFieldDecorator(id, {
          ...
          rules: [
            ...
            { pattern: rule && rule.pattern, message: `${rule && rule.message}`},
          ],
        })(
    ...
    return (
      ...
      >
        {form.getFieldDecorator(id, {
          ...
          <Select diabled={disabled} onChange ={this.props.onChange}>
            {
              options && options.map((data, index) => (
                <Option key={index} value={data.id}>{data.name || data.value || data.catName}</Option>
              ))
            }
          </Select>,
        )}
...
export class ItemRadio extends PureComponent {
  ...
  render() {
    ...
    let newDefaultValue = defaultValue ? defaultValue.id : '';
    return (
      <FormItem
        label={name}
        {...layout}
